...
# @router.post("/ask")
# def ask_eligibility(payload: dict, db: Session = Depends(get_db),  current_user: User = Depends(get_current_user)):
...
# @router.get("/history")
# def eligibility_history(document_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def get_eligibility_history(
    ...
    if not doc:
        raise HTTPException(status_code=404, detail="Eligibility document not found")
...
@router.post("/ask")
def ask_eligibility(payload: dict):
    return check_eligibility(payload.get("question"))